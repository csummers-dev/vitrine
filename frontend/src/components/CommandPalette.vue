...
// Calm Minimal: palette row icons are a single muted ink (chrome), matching the
// sidebar / toolbar — bound inline on the row so it wins over the base
// `.cmd-palette__row-icon` rule. The old per-icon 6-color "vivid" map is gone;
// the selected row's accent highlight provides the only color.
